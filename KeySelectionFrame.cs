...
public partial class KeySelectionFrame : Form
{
    ...
    {
        if (textBox1.Tag is not AshKey userPress || textBox2.Tag is not AshKey willPress)
        {
            MessageBox.Show("请先选择按键喔 :(");
            return;
        }

        mainListView.Items.Add(AshUtil.CreateKeyMappingItem(userPress, willPress, comboBox1.SelectedItem.ToString(), comboBox1.SelectedIndex));
