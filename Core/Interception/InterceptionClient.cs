...
public class InterceptionClient : IDisposable
{
    ...
    {
        deviceHandle = CreateFile(DevicePath, GENERIC_READ | GENERIC_WRITE, 0, IntPtr.Zero, OPEN_EXISTING, 0, IntPtr.Zero);
    ...
    private void SendInput<T>(T stroke) where T : struct
    {
        ...
        try
        {
            ...
            if (!DeviceIoControl(deviceHandle, InterceptionConstants.IOCTL_WRITE, buffer, (uint)size, IntPtr.Zero, 0, out _, IntPtr.Zero))
            {
